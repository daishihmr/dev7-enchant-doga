...
interface Formatter {
	boolean format(JSON json, Context context, Object src, Object o,
			OutputSource out) throws Exception;
}

final class NullFormatter implements Formatter {
	public static final NullFormatter INSTANCE = new NullFormatter();

	public boolean format(final JSON json, final Context context,
			final Object src, final Object o, final OutputSource out)
			throws Exception {
		out.append("null");
		return false;
	}
...
final class StringFormatter implements Formatter {
	...
	static void serialize(final Context context, final String s,
			final OutputSource out) throws Exception {
		serialize(context, s, out, true);
	}

	static void serialize(final Context context, final String s,
			final OutputSource out, boolean quat) throws Exception {
		if (quat)
			out.append('"');
		int start = 0;
			...
			if (c < ESCAPE_CHARS.length) {
				...
				if (x == 0) {
					...
					start = i + 1;
						...
						|| (x == -2 && context.getMode() == Mode.SCRIPT)) {
					if (start < i)
						out.append(s, start, i);
					out.append("\\u00");
...
final class TimeZoneFormatter implements Formatter {
	...
	public boolean format(final JSON json, final Context context,
			final Object src, final Object o, final OutputSource out)
			throws Exception {
		return StringFormatter.INSTANCE.format(json, context, src,
	...
	public boolean format(final JSON json, final Context context,
		...
		try {
			String text = (String) inetAddressClass.getMethod("getHostAddress")
					.invoke(o);
			return StringFormatter.INSTANCE.format(json, context, src, text,
					out);
		} catch (Exception e) {
...
final class EnumFormatter implements Formatter {
	...
	public boolean format(final JSON json, final Context context,
		...
		if (context.getEnumStyle() != null) {
			return StringFormatter.INSTANCE.format(json, context, src, context
					.getPropertyStyle().to(((Enum<?>) o).name()), out);
		} else {
			return NumberFormatter.INSTANCE.format(json, context, src,
					((Enum<?>) o).ordinal(), out);
		}
	}
}
...
final class FloatFormatter implements Formatter {
	...
	public boolean format(final JSON json, final Context context,
		...
		NumberFormat f = context.getNumberFormat();
		if (f != null) {
			StringFormatter.serialize(context, f.format(o), out, false);
		} else {
...
final class CalendarFormatter implements Formatter {
	...
	public boolean format(final JSON json, final Context context,
			final Object src, final Object o, final OutputSource out)
			throws Exception {
		return DateFormatter.INSTANCE.format(json, context, src,
...
final class SQLArrayFormatter implements Formatter {
	...
	public boolean format(final JSON json, final Context context,
		...
		if (array == null)
			array = new Object[0];
		return ObjectArrayFormatter.INSTANCE.format(json, context, src, array,
				out);
	}
}
...
final class LocaleFormatter implements Formatter {
	...
	public boolean format(final JSON json, final Context context,
			final Object src, final Object o, final OutputSource out)
			throws Exception {
		return StringFormatter.INSTANCE.format(json, context, src, ((Locale) o)
...
final class IterableFormatter implements Formatter {
	...
	public boolean format(final JSON json, final Context context,
			final Object src, final Object o, final OutputSource out)
			throws Exception {
		return IteratorFormatter.INSTANCE.format(json, context, src,
...
final class DOMDocumentFormatter implements Formatter {
	...
	public boolean format(final JSON json, final Context context,
			final Object src, final Object o, final OutputSource out)
			throws Exception {
		return DOMElementFormatter.INSTANCE.format(json, context, src,
